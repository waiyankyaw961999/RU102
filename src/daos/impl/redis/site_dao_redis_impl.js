...
const findAll = async () => {
  ...
  const siteIDsKeyArray = await client.smembersAsync(siteIDsKey);
  if (siteIDsKeyArray.length > 0) {
    for (let i = 0; i < siteIDsKeyArray.length; i++) {
      const site = await client.hgetallAsync(siteIDsKeyArray[i]);
      sites[i] = remap(site);
    }
  }
  return sites;
